...
func (s *FilmSequelService) GetAll(filmId string) ([]Film, error) {
	result, err := s.filmSequelRepo.GetAll(context.Background(), filmId)

	if err != nil {
		return []Film{}, fmt.Errorf("failed to fetch sequel from repository: %w", err)
	}

	if len(result) > 0 {
		var sequels []Film
		for _, sequel := range result {
			res, err := s.filmService.GetOne(strconv.Itoa(sequel.FilmId))

			if err != nil {
				return []Film{}, fmt.Errorf("failed to fetch sequel from Kinopoisk API: %w", err)
			}
...
func (s *FilmSequelService) FetchSequels(filmId string) ([]Film, error) {
	...
	if resAllSequels.StatusCode != http.StatusOK {
		return []Film{}, fmt.Errorf("kinopoisk API request failed with status: %d", resAllSequels.StatusCode)
	}
	...
	for _, sequel := range externalSequels {
		...
		if err != nil {
			return []Film{}, fmt.Errorf("failed to convert filmId to int: %w", err)
		}
	...
	if err != nil {
		return []Film{}, fmt.Errorf("failed to decode response from Kinopoisk API: %w", err)
	}
	return sequels, nil
}
